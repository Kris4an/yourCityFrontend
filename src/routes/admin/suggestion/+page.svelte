<script lang="ts">
    ...
    const sortOptionsBG = ['По дата намаляващо', 'По дата увеличаващо', 'По харесвания намаляващо', 'По харесвания увеличаващо']
    ...
    async function getSuggestions(){
        ...
        const res = await fetch(PUBLIC_API_URL+"/admin/suggestion/all?page="+page+"&sort="+sortR+statusF,{
            method: 'GET',
            credentials: 'include'
        })
        if(res.status == 200){
			...
			suggestions = temp.filter(
				(obj, index, self) =>
					index ===
					self.findIndex(
						(o) => o.suggestionId === obj.suggestionId
					)
			);
        }
    }
	const adminUserAndSuggestionExample = {
    suggestionId: 1,
    title: "Подобрете UI на Уебсайта",
    content: "Оптимизирайте потребителския опит с модерен и интуитивен дизайн. Добавете функционалности за по-добра взаимодействие.",
    postDate: "2024-02-16",
    isAnon: false,
    status: "waiting",
    category: "Потребителски Интерфейс",
    user: {
        userId: 123,
        userName: "johnDoe",
        role: "Администратор",
        email: "john.doe@example.com",
        school: "Технически Университет",
		phone: '0888123456'
    },
    likes: 10
};
...
<div id="holder">
	...
	<AdminSuggestionComponent suggestion={adminUserAndSuggestionExample}/>
